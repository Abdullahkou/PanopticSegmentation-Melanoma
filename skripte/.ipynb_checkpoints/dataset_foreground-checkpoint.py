...
class NucleiSegmentationDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if self.ttrain and self.use_aug_data:
            ...
            self.patch_info = pd.concat([self.patch_info, augmented_patch_info], ignore_index=True)
    ...
    def color_image_to_mask(self, color_image):
        ...
        (255, 0, 255): 1,      # TILs (lymphocytes and plasma cells)
        (150, 200, 150): 1,    # Other cells (histiocytes, stromal cells, etc.)
        (0, 255, 0): 1,
        (51, 0, 51): 1,
        (0, 128, 128): 1,
        (204, 204, 51): 1,
    ...
    def __getitem__(self, idx):
     ...
     # Falls aktiviert, Graustufenkanal hinzufügen und auf [0, 1] skalieren
        if self.add_gray_channel:
            ...
            #gray_image = torch.tensor(foreground_mask, dtype=torch.float32) / 255.0  # Auf [0, 1] skalieren
