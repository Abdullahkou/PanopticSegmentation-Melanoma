...
import random
from he_randaugment.augmenters.color.hedcoloraugmenter import HedColorAugmenter
from he_randaugment.augmenters.color.hsbcoloraugmenter import HsbColorAugmenter
...
def hsv(image, factor):
    ...
    augmentor = HsbColorAugmenter(hue_sigma_range=(-factor, factor),
                                  saturation_sigma_range=(-factor, factor), brightness_sigma_range=(0, 0))
...
def translate_x(image, pixels, replace):
    ...
    image = image.transform(image.size, Image.AFFINE,
                            (1, 0, pixels, 0, 1, 0), fillcolor=replace)
    return np.asarray(image)
...
def translate_y(image, pixels, replace):
    ...
    image = Image.fromarray(image)
    image = image.transform(image.size, Image.AFFINE,
                            (1, 0, 0, 0, 1, pixels), fillcolor=replace)
    return np.asarray(image)
...
def shear_x(image, level, replace):
    ...
    #  0  1].
    image = Image.fromarray(image)
    image = image.transform(image.size, Image.AFFINE, (1,
                            level, 0, 0, 1, 0), Image.BICUBIC, fillcolor=replace)
    return np.asarray(image)
...
def shear_y(image, level, replace):
    ...
    image = Image.fromarray(image)
    image = image.transform(image.size, Image.AFFINE, (1,
                            0, 0, level,  1, 0), Image.BICUBIC, fillcolor=replace)
    return np.asarray(image)
...
def distort_image_with_randaugment(image, num_layers, magnitude, ra_type):
    ...
    #     cutout_const=40, translate_const=10)
    # The 'Default' option is the H&E tailored randaugment
    if ra_type == 'Default':
        available_ops = ['Brightness',
                         'Sharpness', 'Color', 'Contrast', 'Equalize', 'Identity', 'Hsv', 'Hed']
    elif ra_type == 'Original':
        available_ops = ['ShearX', 'ShearY', 'Brightness', 'Sharpness',
                         'Color', 'Contrast', 'Rotate', 'Equalize', 'Identity', 'Solarize', 'Posterize', 'Invert']
