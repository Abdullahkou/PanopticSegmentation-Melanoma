...
from model_foreground import ForeUNetPlusPlus
...
class NucleiSegmentationDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.foreground_model = ForeUNetPlusPlus(2, 4)
        self.foreground_model.load_state_dict(torch.load(os.path.join(r"../foreground/logs/foreground_seg9", r"best_f1.pt")))
        self.foreground_model = nn.Sequential(self.foreground_model, nn.Sigmoid())
#        self.foreground_model = self.foreground_model.cuda()
    ...
    def get_foreground(self, model, image, img):

        gray_image = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)  # Graustufenkanal erzeugen
        ...
        image = torch.cat((image, gray_image), dim=0)
    ...
    def __getitem__(self, idx):
     ...
     # Falls aktiviert, Graustufenkanal hinzufügen und auf [0, 1] skalieren
        if self.add_gray_channel:
            foreground_mask = self.get_foreground(self.foreground_model, rgb_image, image)
            gray_image = torch.tensor(foreground_mask, dtype=torch.float32)
