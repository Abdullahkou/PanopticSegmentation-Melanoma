...
def loss_old(y_pred: torch.tensor, y_true: torch.tensor):
    
    class_weights = torch.tensor([0.0, 420.0 , 2624.0, 138.0 ], dtype=torch.float32)
...
def loss(y_pred: torch.Tensor, y_true: torch.Tensor):
    ...
    # Sicherstellen, dass class_weights auf demselben Gerät wie y_pred sind
    device = y_pred.device
    class_weights = class_weights.to(device)
    ...
    ce_loss_fn = nn.CrossEntropyLoss(weight=class_weights, ignore_index=0)
    ce_loss = ce_loss_fn(y_pred.float(), y_true.long())

    # Focal Loss Berechnung
    gamma = 2.0
    p_t = F.softmax(y_pred, dim=-1)
    p_t = p_t.gather(1, y_true.unsqueeze(1)).squeeze(1)
    focal_loss = -(1 - p_t) ** gamma * torch.log(p_t + 1e-8)
    focal_loss = focal_loss.mean()

    # Dice Loss Berechnung (für Multi-Klassen-Segmentierung)
    y_pred_softmax = F.softmax(y_pred, dim=-1)
    y_true_one_hot = F.one_hot(y_true, num_classes=c).float()

    intersection = torch.sum(y_pred_softmax * y_true_one_hot, dim=0)
    union = torch.sum(y_pred_softmax, dim=0) + torch.sum(y_true_one_hot, dim=0)

    dice_loss = 1 - (2. * intersection + 1e-8) / (union + 1e-8)
    dice_loss = dice_loss.mean()

    # Kombinierte Loss-Funktion
    total_loss = 0.5 * ce_loss + 0.3 * focal_loss + 0.2 * dice_loss
