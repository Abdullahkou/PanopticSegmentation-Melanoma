...
def focal_loss(y_pred: torch.Tensor, y_true: torch.Tensor, alpha: torch.Tensor = torch.tensor([0.1, 1.0, 2.0, 3.0]), gamma: float = 2.0, t_type=None):
    """
    Verlustfunktion mit Focal Loss nach deinem vorgegebenen Muster.
    """
    device = y_pred.device
    class_weights = torch.tensor([0.3, 1.0, 2.0, 5.0]).to(device)  # Klassen-Gewichte
    ...
    if t_type == "nuc":
        ...
        class_weights = torch.tensor([0.01, 2.0, 5.0, 1.0]).to(device)  # Klassen-Gewichte
        ce_loss = F.nll_loss(log_prob, y_true.long(), weight=class_weights , reduction='none')
...
def loss(y_pred: torch.tensor, y_true: torch.tensor,t_type=None):
    return focal_loss(y_pred, y_true, t_type=t_type)
