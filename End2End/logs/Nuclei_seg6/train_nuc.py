...
if __name__ == "__main__":
    ...
    model = HalfDualDecUNetPlusPlus(in_channels=in_channels)
    
    
    model.to(device)

    # Funktion zur Erstellung von Parametergruppen
    def create_param_groups(module):
        g0, g1, g2 = [], [], []  # Parametergruppen
        for v in module.modules():
            if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):  # Bias
                g2.append(v.bias)
            if isinstance(v, nn.BatchNorm2d):  # BatchNorm ohne Gewichtungsabfall
                g0.append(v.weight)
            elif hasattr(v, 'weight') and isinstance(v.weight, nn.Parameter):  # Gewicht mit Gewichtungsabfall
                g1.append(v.weight)
        return g0, g1, g2
    
    
    # Parametergruppen für die beiden Netzwerke
    g0_net1, g1_net1, g2_net1 = create_param_groups(model.net1)
    g0_net2, g1_net2, g2_net2 = create_param_groups(model.net2)
    
    # Optimierer für das erste Netzwerk erstellen
    optimizer_net1 = torch.optim.Adam(g0_net1, lr=trial_spec['learning_rate'])
    optimizer_net1.add_param_group({'params': g1_net1, 'weight_decay': trial_spec['weight_decay']})
    optimizer_net1.add_param_group({'params': g2_net1})  # Bias hinzufügen
    
    # Optimierer für das zweite Netzwerk erstellen
    optimizer_net2 = torch.optim.Adam(g0_net2, lr=trial_spec['learning_rate'])
    optimizer_net2.add_param_group({'params': g1_net2, 'weight_decay': trial_spec['weight_decay']})
    optimizer_net2.add_param_group({'params': g2_net2})  # Bias hinzufügen
    
    # Scheduler für beide Optimierer erstellen
    scheduler_net1 = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_net1,
        T_max=200 * len_data / batch_size
    )
    
    scheduler_net2 = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_net2,
        T_max=200 * len_data / batch_size
    )
    
    # Speicher freigeben
    del g0_net1, g1_net1, g2_net1, g0_net2, g1_net2, g2_net2
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for (x, y) in tqdm(train_dataloader,
            ...
            logits1, output = model(x)
            output.to(device)
            
            loss1 = utils.loss(logits1.permute(0, 2, 3, 1), y)
            loss2 = utils.loss(output.permute(0, 2, 3, 1), y)
        ...
        writer.add_scalar('learning_rate/train',
                          optimizer_net1.__dict__.get('param_groups')[0].get('lr'),
                          epoch)
