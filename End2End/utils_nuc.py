...
def focal_loss(y_pred: torch.Tensor, y_true: torch.Tensor, alpha: torch.Tensor = torch.tensor([0.1, 1.0, 2.0, 3.0]), gamma: float = 2.0):
    """
    Verlustfunktion mit Focal Loss nach deinem vorgegebenen Muster.
    """
    device = y_pred.device
    class_weights = torch.tensor([0.1, 1.0, 2.0, 3.0]).to(device)  # Klassen-Gewichte
