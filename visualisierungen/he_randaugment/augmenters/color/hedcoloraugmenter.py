...
"""
from he_randaugment.augmenters.color.utils.custom_hed_transform import rgb2hed, hed2rgb
...
class HedColorAugmenter(dptcoloraugmenterbase.ColorAugmenterBase):
    ...
    def __init__(self, haematoxylin_sigma_range, haematoxylin_bias_range, eosin_sigma_range, eosin_bias_range, dab_sigma_range, dab_bias_range, cutoff_range):
        ...
        # Initialize members.
        #
        # Configured sigma ranges for H, E, and D channels.
        self.__sigma_ranges = None
        # Configured bias ranges for H, E, and D channels.
        self.__bias_ranges = None
        self.__cutoff_range = None  # Cutoff interval.
        # Randomized sigmas for H, E, and D channels.
        self.__sigmas = None
        # Randomized biases for H, E, and D channels.
        self.__biases = None

        # Save configuration.
        #
        self.__setsigmaranges(haematoxylin_sigma_range=haematoxylin_sigma_range,
                              eosin_sigma_range=eosin_sigma_range, dab_sigma_range=dab_sigma_range)
        self.__setbiasranges(haematoxylin_bias_range=haematoxylin_bias_range,
                             eosin_bias_range=eosin_bias_range, dab_bias_range=dab_bias_range)
    ...
    def __setsigmaranges(self, haematoxylin_sigma_range, eosin_sigma_range, dab_sigma_range):
        ...
        if haematoxylin_sigma_range is not None:
            if len(haematoxylin_sigma_range) != 2 or haematoxylin_sigma_range[1] < haematoxylin_sigma_range[0] or haematoxylin_sigma_range[0] < -1.0 or 1.0 < haematoxylin_sigma_range[1]:
                raise Exception(
                    "InvalidHaematoxylinSigmaRangeError(haematoxylin_sigma_range)")
        ...
        if eosin_sigma_range is not None:
            if len(eosin_sigma_range) != 2 or eosin_sigma_range[1] < eosin_sigma_range[0] or eosin_sigma_range[0] < -1.0 or 1.0 < eosin_sigma_range[1]:
                raise Exception(
                    "InvalidEosinSigmaRangeError(eosin_sigma_range)")
        ...
        # Store the settings.
        #
        self.__sigma_ranges = [haematoxylin_sigma_range,
                               eosin_sigma_range, dab_sigma_range]
    ...
    def __setbiasranges(self, haematoxylin_bias_range, eosin_bias_range, dab_bias_range):
        ...
        if haematoxylin_bias_range is not None:
            if len(haematoxylin_bias_range) != 2 or haematoxylin_bias_range[1] < haematoxylin_bias_range[0] or haematoxylin_bias_range[0] < -1.0 or 1.0 < haematoxylin_bias_range[1]:
                raise Exception(
                    "InvalidHaematoxylinBiasRangeError(haematoxylin_bias_range)")
        ...
        # Store the settings.
        #
        self.__bias_ranges = [haematoxylin_bias_range,
                              eosin_bias_range, dab_bias_range]
    ...
    def __setcutoffrange(self, cutoff_range):
        ...
        # Store the setting.
        #
        self.__cutoff_range = cutoff_range if cutoff_range is not None else [
            0.0, 1.0]
    ...
    def randomize(self):
        ...
        #
        self.__sigmas = [np.random.uniform(low=sigma_range[0], high=sigma_range[1], size=None)
                         if sigma_range is not None else 1.0 for sigma_range in self.__sigma_ranges]
        self.__biases = [np.random.uniform(low=bias_range[0], high=bias_range[1], size=None)
                         if bias_range is not None else 0.0 for bias_range in self.__bias_ranges]
